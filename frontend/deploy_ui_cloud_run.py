...
def GenerateConfig(context):
    ...
    if api_domain_name is not "":
        ...
        if domain_has_protocol(api_domain_name):
            environment_variables = f'VUE_APP_API_BASE_URL={api_domain_name}{base_path}'
        else:
            # if a user includes the UI domain name then include it as an environment variable
            domain_protocol = 'https://'
            environment_variables = f'VUE_APP_API_BASE_URL={domain_protocol}{api_domain_name}{base_path}'

    # set the Project ID and Client ID environment variables
    environment_variables += ',VUE_APP_PROJECT_ID=$PROJECT_ID'
    environment_variables += f',VUE_APP_GOOGLE_APP_CLIENT_ID={client_id}'
    steps[2]['args'].append(f'--set-env-vars={environment_variables}')
        ...
        git_release = {
            ...
            'args': [
                ...
                (
                    (
                        (
                            f'if ! gcloud container images describe gcr.io/$PROJECT_ID/{datashare_ui_name_with_tag}; then '
                            + 'git checkout '
                        )
                        + git_release_version
                    )
